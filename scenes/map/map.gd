...
func _ready() -> void:
	camera_edge_y = MapGenerator.Y_DIST * (MapGenerator.FLOORS - 1)
		# Wait a frame so map data is generated before trying to select
	await get_tree().process_frame
	
	# Auto-select first available room (on the first floor)
	_auto_select_first_room()
...
func _auto_select_first_room() -> void:
	for map_room: MapRoom in rooms.get_children():
		if map_room.room.row == 0:
			if map_room.available:
				# Simulate player clicking and selecting this room
				_on_map_room_clicked(map_room.room)
				_on_map_room_selected(map_room.room)
				break
